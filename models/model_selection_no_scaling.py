...
def save_results(results_df, target_name):
    ...
    # Save as CSV with no_scaling suffix
    csv_path = os.path.join(plots_dir, f'model_comparison_{target_name}_no_scaling.csv')
    results_df_rounded.to_csv(csv_path, index=False)
    
    # Create and save bar plot with no_scaling suffix
    plt.figure(figsize=(12, 6))
    sns.barplot(data=results_df, x='Model', y='R2')
    plt.title(f'Model Comparison for {target_name.capitalize()} Prediction (No Scaling)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'model_comparison_{target_name}_no_scaling.png'))
    plt.close()
    
    print(f"\nResults saved to {plots_dir}/")
    print(f"- model_comparison_{target_name}_no_scaling.csv")
    print(f"- model_comparison_{target_name}_no_scaling.png")
...
def plot_feature_importance(model, feature_names, target_name, model_name):
    ...
    plt.title(f'Feature Importance for {model_name} ({target_name.capitalize()}) - No Scaling')
    plt.tight_layout()
    
    # Save plot with no_scaling suffix
    plt.savefig(os.path.join(plots_dir, f'feature_importance_{target_name}_{model_name.replace(" ", "_")}_no_scaling.png'))
    plt.close()
    
    # Save importance values to CSV with no_scaling suffix
    importance_df.to_csv(
        os.path.join(plots_dir, f'feature_importance_{target_name}_{model_name.replace(" ", "_")}_no_scaling.csv'),
        index=False
    )
    
    print(f"Feature importance plot saved to plots/feature_importance_{target_name}_{model_name.replace(' ', '_')}_no_scaling.png")

def main():
    # Load data
    valence_data, arousal_data = load_data()
    
    # Process and evaluate models for valence (no scaling)
    print("\nValence Prediction Models (No Scaling):")
    X_train_v, X_test_v, y_train_v, y_test_v, scaler_v = prepare_data(valence_data, 'valence', never_scale=True)
    ...
    # Process and evaluate models for arousal (no scaling)
    print("\nArousal Prediction Models (No Scaling):")
    X_train_a, X_test_a, y_train_a, y_test_a, scaler_a = prepare_data(arousal_data, 'arousal', never_scale=True)
    ...
    # Save the best models with no_scaling suffix
    models_dir = os.path.join(os.getcwd(), 'models', 'trained')
    os.makedirs(models_dir, exist_ok=True)
    
    joblib.dump(best_model_v_tuned, os.path.join(models_dir, 'best_valence_model_no_scaling.joblib'))
    joblib.dump(best_model_a_tuned, os.path.join(models_dir, 'best_arousal_model_no_scaling.joblib'))
    
    print("\nBest models have been saved in the models/trained directory with _no_scaling suffix.")
